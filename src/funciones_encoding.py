...
class TestEstadisticos:
    ...
    def comprobar_pvalue(self, pvalor):
        ...
        """
        if pvalor < 0.05:
            print(">>>>>>>>> HAY DIFERENCIAS SIGNIFICATIVAS ENTRE LOS DATOS\n")
        else:
    ...
    def run_all_tests(self):
        ...
        if test_choice.lower()=="m":
            test_choice="mannwhitneyu"

        elif test_choice.lower()=="w":
            test_choice="wilcoxon"

        elif test_choice.lower()=="k":
            test_choice="kruskal"

        elif test_choice.lower()=="a":
            test_choice="anova"

        else:
            print(f"La opción '{test_choice}' no es válida")
            return
    ...
    def detectar_orden_problema_categorico(self):
        
        if self.columna_categorica != self.variable_respuesta:
            print(f"Estamos comparando las variables de {self.columna_categorica.upper()}")
            df_cross_tab = pd.crosstab(self.dataframe[self.columna_categorica], self.dataframe[self.variable_respuesta])
            ...
            if p<0.05:
                print(f"La variable {self.columna_categorica.upper()} SÍ tiene orden ✅")
                print("\n---------------------------------------------\n")
                return True
            else:
                print(f"La variable {self.columna_categorica.upper()} NO tiene orden ❌")
                print("\n---------------------------------------------\n")
                return False
