...
class GestionOutliersMultivariados:
    ...
    def detectar_outliers_if(self, cols, vr, contaminacion, estimadores, visualizar=True):
        """
        ...
        df_outliers = self.dataframe.copy()

        for contaminacion, estimador in combinaciones:
            ifo = IsolationForest(random_state = 42, #semilla
                                n_estimators = estimador,  # número de árboles
                                contamination = contaminacion,
                                n_jobs = -1 # utilice todos los núcleos
                                )
            df_outliers[f"outliers_ifo_{contaminacion}_{estimador}"] = ifo.fit_predict(self.dataframe[cols])
        
        if visualizar:
            self.visualizar_outliers(df_outliers, cols, vr)
    ...
    # def visualizar_outliers_vieja(self, df, cols, vr):
    #     cols.append(vr)
    #     combinaciones_variables = list(combinations(cols, 2))
    #     columnas_hue = df.filter(like="outlier").columns

    #     for col_hue in columnas_hue:
    #         fig, axes = plt.subplots(ncols=len(cols), nrows=1, figsize=(15,5))
    #         axes = axes.flat

    #         for indice, tupla in enumerate(combinaciones_variables):
    #             sns.scatterplot(df, x=tupla[0], y=tupla[1], ax=axes[indice], hue = col_hue, palette="Set1", style=col_hue, alpha=0.2)
    #             plt.suptitle(f"{col_hue}")
    #             plt.tight_layout()

    def visualizar_outliers(self, df, cols, vr):
        cols.append(vr)
        combinaciones_variables = list(combinations(cols, 2))
        columnas_hue = df.filter(like="outlier").columns

        for col_hue in columnas_hue:
            # Ajusta el número de filas y columnas de subgráficos dinámicamente
            num_combinaciones = len(combinaciones_variables)
            n_cols = 3  # Número de columnas fijas para los gráficos
            n_rows = ceil(num_combinaciones / n_cols)  # Calcula las filas necesarias

            fig, axes = plt.subplots(ncols=n_cols, nrows=n_rows, figsize=(15, 5 * n_rows))
            axes = axes.flat  # Aplanar para acceder por índice

            for indice, tupla in enumerate(combinaciones_variables):
                if indice < len(axes):  # Solo intentar graficar si el índice es válido
                    sns.scatterplot(
                        data=df,
                        x=tupla[0],
                        y=tupla[1],
            ...
            # Ajustar diseño y título
            plt.suptitle(f"{col_hue}")
            plt.tight_layout()

            # Si hay más combinaciones que subgráficos, esconder ejes vacíos
            for ax in axes[len(combinaciones_variables):]:
